...
subroutine sync_from_change_pos(npart_in, nusedpart_in, epsilon_in, fudgepsilon_in, xlong_in, boxsize2_in)
   ...
   integer(kind=4), intent(in) :: npart_in, nusedpart_in
...
!=======================================================================
   ...
   real(kind=8)                :: densest
   integer(kind=4), allocatable :: path(:)
   integer(kind=4) :: npath, k, groupid
   integer(kind=4) :: tttt0, tttt1, ttttrate
   real(kind=8)    :: dtdtdtdt
...
!=======================================================================
   ...
   !$OMP SINGLE
   call dev_create_nodes_task(rhot,inode,igr1,igr2,.false.)
   !$OMP END SINGLE
   !$OMP END PARALLEL
...
!=======================================================================
subroutine dev_do_colorize_local(igroup, igr, rhot, queue)
!=======================================================================
   implicit none
   ...
   do while (front < back)
      ...
      do ineig = 1, neig
         ...
         if (dev_rho_saddle_all(idx) > rhot) then
            ...
            !    write(errunit,*) 'ERROR in dev_do_colorize_local : color(igr2) <> icolor_select'
...
!=======================================================================
recursive subroutine dev_create_nodes_task(rhot0,inode0,igr1_0,igr2_0,do_paint)
!=======================================================================
   implicit none
...
!     ------------------------------------------------------------
      ...
      do igr=igr1,igr2
         ...
         if (color(igr).eq.0) then
            ...
            call dev_do_colorize_local(igroup,igr,rhot,queue_local)
         endif
      enddo
...
&           igrpos(icolor)=igrpos(icolor)+1
      ...
      if (igrpos(icolor_select)-igr1+1.eq.0) then
         write(errunit,*) 'ERROR in dev_create_nodes_task :'
...
!        Spawn child subtree tasks
         do isisters=1,nsisters
            ...
            call dev_create_nodes_task(t_rhotout,t_inodeout,t_igr1out,t_igr2out,.true.)
            !$OMP END TASK
         enddo
...
end subroutine dev_create_nodes_task
...
!=======================================================================
   ...
   do igroup1=1,ngroups
      ...
      if (neig.gt.0) then
         ...
         dev_rho_saddle_all(base:base+ineig-1)=rho_sad(1:ineig)
